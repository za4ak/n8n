...
describe('toolsAgentExecute V3', () => {
	...
	it('should handle streaming with tool calls detected', async () => {
		...
		mockContext.getNode.mockReturnValue(mockNode);
		mockContext.getInputData.mockReturnValue([{ json: { text: 'test input' } }]);
		mockContext.getSubnodes.mockReturnValue(['TestTool']);
		...
		const result = (await toolsAgentExecute.call(mockContext)) as Request<RequestResponseMetadata>;
	...
	it('should handle response with previous tool calls', async () => {
		...
		const response: Response<RequestResponseMetadata> = {
			actionResponses: [
				{
					action: {
						id: 'call_123',
